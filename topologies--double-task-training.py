...
class EnhancedDebugCallback(BaseCallback):
    ...
    def __init__(self, verbose=0, wandb_run=None, log_freq=100):
        ...
        self.last_logged_step = 0  # Track the last step we logged to avoid duplicates
    ...
    def _on_step(self) -> bool:
        ...
        # Log detailed metrics every log_freq steps, but only if we haven't logged this step
        if (self.step_count % self.log_freq == 0 and 
            self.wandb_run is not None and 
            self.num_timesteps > self.last_logged_step):
    ...
    def _on_rollout_end(self) -> None:
        ...
        try:
            ...
            print(f"   ⚠️  Error in rollout end callback: {e}")
            import traceback
            traceback.print_exc()
    ...
    def _on_training_end(self) -> None:
        ...
        try:
            ...
            print(f"   ⚠️  Error in training end callback: {e}")
            import traceback
            traceback.print_exc()
                    ...
                    if hasattr(self.model, 'policy') and hasattr(self.model.policy, 'actor_topology'):
                        ...
                        metrics.update({
                            ...
                            "network/total_parameters": actor_params + critic_params,
                                    ...
                                    # Only log if we haven't logged this step before to avoid step ordering issues
                if self.num_timesteps > self.last_logged_step:
        ...
        try:
            ...
            if hasattr(self.model, 'rollout_buffer') and self.model.rollout_buffer is not None:
                ...
                if hasattr(buffer, 'rewards') and len(buffer.rewards) > 0:
                    ...
                    # Log rollout metrics only if we haven't logged this step before
                    if self.num_timesteps > self.last_logged_step:
                        self.wandb_run.log(metrics, step=self.num_timesteps)
                        self.last_logged_step = self.num_timesteps
