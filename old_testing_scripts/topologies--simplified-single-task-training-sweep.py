#!/usr/bin/env python3
"""
Simplified Single-Task Training Sweep for Topology Networks
...
Cross-task evaluation is performed on both CartPole and Acrobot.
...
def simplified_single_task_training(policy_class, topology_type, config, num_layers=2, hidden_size=None, train_task=None):
    """
    Simplified single-task training function with sweep support.
    
    This function trains on one task and evaluates on both CartPole and Acrobot.
    ...
    """
    print(f"🎯 SIMPLIFIED SINGLE-TASK TRAINING: {topology_type.upper()} TOPOLOGY")
    ...
    if wandb.run is None:
        wandb.init(
            project="topologies--simplified-single-task-training",
            entity="katko-it-universitetet-i-k-benhavn",
            config=config,
            name=f"simplified_single_task_{topology_type}_{train_task}"
        )
    ...
    # EVALUATION: Test on both CartPole and Acrobot
    print(f"📊 Evaluating on both tasks...")
    
    # Test on training task
    eval_env_train = make_env(train_task)()
    rewards_train, lengths_train, success_train = evaluate_model_enhanced(
        model, eval_env_train, train_task, config['n_eval_episodes']
    )
    
    # Test on the other task (cross-task evaluation)
    other_task = 'Acrobot-v1' if train_task == 'CartPole-v1' else 'CartPole-v1'
    eval_env_other = make_env(other_task)()
    rewards_other, lengths_other, success_other = evaluate_model_enhanced(
        model, eval_env_other, other_task, config['n_eval_episodes']
    )
    
    # Log combined results
    if wandb.run:
        wandb.log({
            'testing/mean_reward': (np.mean(rewards_train) + np.mean(rewards_other)) / 2,
            'testing/train_task_mean_reward': np.mean(rewards_train),
            'testing/other_task_mean_reward': np.mean(rewards_other),
            'testing/train_task_success_rate': success_train,
            'testing/other_task_success_rate': success_other,
            'testing/overall_success_rate': (success_train + success_other) / 2,
            'testing/train_task': train_task,
            'testing/other_task': other_task,
    ...
    eval_env_other.close()
    
    return {
        'train_task_rewards': rewards_train,
        'other_task_rewards': rewards_other,
        'train_task_success': success_train,
        'other_task_success': success_other,
        'simplified_mode': True
    }
...
def train_with_sweep():
    ...
    # Run training
    result = simplified_single_task_training(
    ...
    print("✅ Simplified single-task training completed!")
    return result
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Simplified Single-Task Training Sweep")
