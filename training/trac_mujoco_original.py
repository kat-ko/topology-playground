...
train_epochs = 40
max_timesteps = 1000
state_scale = 1.0  # Will be replaced with proper normalization
reward_scale = 1.0
batch_size = 32
gamma = 0.95
lambd = 0.95
...
def normalize_observation(obs, obs_mean=None, obs_std=None, epsilon=1e-8):
    """
    Normalize observations using running statistics.
    For HalfCheetah, we use a simple normalization approach.
    """
    if obs_mean is None or obs_std is None:
        # Use simple normalization based on typical HalfCheetah observation ranges
        # These are approximate values based on the environment
        obs_mean = np.zeros(obs.shape[-1])
        obs_std = np.ones(obs.shape[-1]) * 0.1  # Small std to prevent division by zero
    
    # Normalize: (obs - mean) / (std + epsilon)
    normalized = (obs - obs_mean) / (obs_std + epsilon)
...
class PolicyNetwork(torch.nn.Module):
    def __init__(self, n=4, in_dim=128, action_type='discrete'):
        ...
        if action_type == 'discrete':
            ...
            self.fc4 = torch.nn.Linear(128, n)  # mean
            self.fc5 = torch.nn.Linear(128, n)  # log_std
        self.l_relu = torch.nn.LeakyReLU(0.1)
        
        # Initialize weights properly for continuous control
    ...
    def _init_weights(self, m):
        if isinstance(m, torch.nn.Linear):
            ...
            # Initialize mean head to output small values for continuous control
            if hasattr(self, 'fc4') and m == self.fc4 and self.action_type == 'continuous':
                torch.nn.init.constant_(m.weight, 0)
                torch.nn.init.constant_(m.bias, 0)
            # Initialize log_std to small values (less exploration initially)
            if hasattr(self, 'fc5') and m == self.fc5:
                torch.nn.init.constant_(m.weight, 0)
                torch.nn.init.constant_(m.bias, -1)  # log_std = -1, so std = 0.37
    ...
    def forward(self, x):
        ...
        if self.action_type == 'discrete':
            ...
            y = torch.cat([mean, log_std], dim=-1)
            return y

    def sample_action(self, state, action_space=None):
        ...
        if self.action_type == 'discrete':
            ...
            mean, log_std = torch.chunk(y, 2, dim=-1)
            ...
            log_probability = dist.log_prob(action).sum(dim=-1)
            return action.squeeze(0).cpu().numpy(), log_probability.item()

    def best_action(self, state, action_space=None):
        if not isinstance(state, torch.Tensor):
            state = torch.from_numpy(state).float().to(device)
    ...
    def evaluate_actions(self, states, actions):
        ...
        if self.action_type == 'discrete':
            ...
            log_probabilities = dist.log_prob(actions).sum(dim=-1)
...
class ValueNetwork(torch.nn.Module):
    ...
    def state_value(self, state):
        ...
        return y.item()
def cumulative_sum(array, gamma=gamma):
    curr = 0
    cumulative_array = []
...
def train_combined_networks(policy_model, value_model, combined_optimizer, data_loader, action_type='discrete', epochs=train_epochs, clip=0.2):
...
def train(env_name, opt_choice, random_perturbations, levels, seed, no_noise = False):
    ...
    trac_reward_log_file = f'iclr/original/mujoco/trac_reward_log_{env_name}_{seed}{suffix}.txt'
    base_reward_log_file = f'iclr/original/mujoco/base_reward_log_{env_name}_{seed}{suffix}.txt'
    ...
    policy_model = PolicyNetwork(in_dim=feature_dim, n=n_actions, action_type=action_type).to(device)

    suffix = "_no_noise" if no_noise else ""
    trac_combined_optimizer = start_trac(log_file=f'iclr/original/mujoco/trac_{env_name}_{seed}{suffix}.text', Base=optim.Adam)(
        [
    ...
    for ite in tqdm_bar:
        ...
        for _ in range(max_episodes):
            ...
            for timestep in range(max_timesteps):
                # Normalize observation properly
                normalized_obs = normalize_observation(observation)
                action, log_probability = policy_model.sample_action(normalized_obs, env.action_space)
                value = value_model.state_value(normalized_obs)
...
# Test to get quick one seed results for HalfCheetah
def test_halfcheetah(levels, seed, no_noise = False):
    peturbations = get_perturbations("HalfCheetah-v4", seed, levels, rp_range = 0 if no_noise else rp_range)
    print("Online Peturbations are")
    print(peturbations)
    env = "HalfCheetah-v4"
    train(env, opt, peturbations, levels, seed, no_noise)
    plot(env, seed, no_noise)

def test_ant(levels, seed, no_noise = False):
    peturbations = get_perturbations("Ant-v4", seed, levels, rp_range = 0 if no_noise else rp_range)
    print("Online Peturbations are")
    print(peturbations)
    env = "Ant-v4"
    train(env, opt, peturbations, levels, seed, no_noise)
...
def read_data(file_path):
    with open(file_path, 'r') as file:
        data = file.readlines()
    return data
def plot(env_name, seed, no_noise = False):
    # Read data from files
    suffix = "_no_noise" if no_noise else ""
    trac_data = read_data(f'iclr/original/mujoco/trac_reward_log_{env_name}_{seed}{suffix}.txt')
    ...
    # save plots
    plt.savefig(f'iclr/original/mujoco/trac_reward_plot_{env_name}_{seed}{suffix}.png')


# main
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='TRAC MuJoCo Training')
    parser.add_argument('--task', type=str, choices=['halfcheetah', 'ant'], 
                       default='halfcheetah', help='Choose task: halfcheetah or ant')
    ...
    if args.task == 'halfcheetah':
        test_halfcheetah(levels, seed, no_noise)
        print(f"Tested HalfCheetah with {levels} levels and seed {seed}" + (" (no noise)" if no_noise else ""))
    elif args.task == 'ant':
        test_ant(levels, seed, no_noise)
        print(f"Tested Ant with {levels} levels and seed {seed}" + (" (no noise)" if no_noise else ""))
