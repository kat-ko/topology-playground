...
def train(env_name, opt_choice, random_perturbations, levels, seed, no_noise = False):
    ...
    trac_reward_log_file = f'iclr/original/control/trac_reward_log_{env_name}_{seed}{suffix}.txt'
    base_reward_log_file = f'iclr/original/control/base_reward_log_{env_name}_{seed}{suffix}.txt'
    ...
    suffix = "_no_noise" if no_noise else ""
    if opt_choice == "TRAC":
        trac_combined_optimizer = start_trac(log_file=f'iclr/original/control/trac_{env_name}_{seed}{suffix}.text', Base=optim.Adam)(
            [
        ...
        print(f"USING TRAC. Log file: iclr/original/control/trac_{env_name}_{seed}{suffix}.text")
    else:  # base optimizer
        ...
        print(f"USING BASE ADAM. Log file: iclr/original/control/base_reward_log_{env_name}_{seed}{suffix}.txt")
    history = History()
    level = 0
    for ite in tqdm_bar:
...
def plot(env_name, seed, no_noise = False, opt_choice = "base"):
    ...
    suffix = "_no_noise" if no_noise else ""
    if opt_choice == "TRAC":
        data_file = f'iclr/original/control/trac_reward_log_{env_name}_{seed}{suffix}.txt'
        label = 'TRAC PPO'
        color = '#b71540'
        plot_file = f'iclr/original/control/trac_reward_plot_{env_name}_{seed}{suffix}.png'
    else:  # base optimizer
        data_file = f'iclr/original/control/base_reward_log_{env_name}_{seed}{suffix}.txt'
        label = 'Adam PPO'
        color = '#4a69bd'
        plot_file = f'iclr/original/control/base_reward_plot_{env_name}_{seed}{suffix}.png'
...
if __name__ == "__main__":
    ...
    if args.task == 'cartpole':
        ...
        print(f"Tested CartPole with {levels} levels, seed {seed}, optimizer {opt}" + (" (no noise)" if no_noise else ""))
    elif args.task == 'acrobot':
        test_acrobot(levels, seed, no_noise)
        print(f"Tested Acrobot with {levels} levels, seed {seed}, optimizer {opt}" + (" (no noise)" if no_noise else ""))
    elif args.task == 'mountaincar':
        test_mountaincar(levels, seed, no_noise)
        print(f"Tested MountainCar with {levels} levels, seed {seed}, optimizer {opt}" + (" (no noise)" if no_noise else ""))
