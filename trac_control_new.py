...
class PolicyNetwork(torch.nn.Module):
    def __init__(self, n=4, in_dim=16):
        super(PolicyNetwork, self).__init__()

        self.fc1 = torch.nn.Linear(in_dim, 16)
        self.fc2 = torch.nn.Linear(16, 16)
        self.fc4 = torch.nn.Linear(16, n)
...
def train(env_name, opt_choice, random_perturbations, levels, seed, no_noise = False):
    ...
    if opt_choice == "TRAC":
        reward_log_file = f'iclr/new/control/trac_reward_log_{env_name}_{seed}{suffix}.txt'
    else:  # base optimizer
        reward_log_file = f'iclr/new/control/base_reward_log_{env_name}_{seed}{suffix}.txt'
    ...
    suffix = "_no_noise" if no_noise else ""
    if opt_choice == "TRAC":
        trac_combined_optimizer = start_trac(log_file=f'iclr/new/control/trac_{env_name}_{seed}{suffix}.text', Base=optim.Adam)(
            [
    ...
    if opt_choice == "TRAC":
        ...
        print(f"USING TRAC. Log file: iclr/new/control/trac_{env_name}_{seed}{suffix}.text")
    else:  # base optimizer
        combined_optimizer = base_combined_optimizer
        print(f"USING BASE ADAM. Log file: iclr/new/control/base_reward_log_{env_name}_{seed}{suffix}.txt")
    history = History()
    level = 0
    ...
    for ite in tqdm_bar:
        ...
        data_loader = DataLoader(history, batch_size=minibatch_size, shuffle=True, collate_fn=lambda x: (
            torch.stack([torch.tensor(item[0], dtype=torch.float32) for item in x]),
...
# Test to get quick one seed results for CartPole
def test_cartpole(levels, seed, no_noise = False):
    peturbations = get_perturbations("CartPole-v1", seed, levels, rp_range = 0 if no_noise else rp_range)
    print("Online Peturbations are")
    print(peturbations)
    env = "CartPole-v1"
    train(env, opt, peturbations, levels, seed, no_noise)
    plot(env, seed, no_noise, opt)
def test_acrobot(levels, seed, no_noise = False):
    peturbations = get_perturbations("Acrobot-v1", seed, levels, rp_range = 0 if no_noise else rp_range)
    print("Online Peturbations are")
    print(peturbations)
    env = "Acrobot-v1"
    train(env, opt, peturbations, levels, seed, no_noise)
    plot(env, seed, no_noise, opt)
def test_mountaincar(levels, seed, no_noise = False):
    peturbations = get_perturbations("MountainCar-v0", seed, levels, rp_range = 0 if no_noise else rp_range)
    print("Online Peturbations are")
    print(peturbations)
    env = "MountainCar-v0"
    train(env, opt, peturbations, levels, seed, no_noise)
...
def plot(env_name, seed, no_noise = False, opt_choice = "base"):
    ...
    suffix = "_no_noise" if no_noise else ""
    if opt_choice == "TRAC":
        data_file = f'iclr/new/control/trac_reward_log_{env_name}_{seed}{suffix}.txt'
        label = 'TRAC PPO'
        color = '#b71540'
        plot_file = f'iclr/new/control/trac_reward_plot_{env_name}_{seed}{suffix}.png'
    else:  # base optimizer
        data_file = f'iclr/new/control/base_reward_log_{env_name}_{seed}{suffix}.txt'
        label = 'Adam PPO'
        color = '#4a69bd'
        plot_file = f'iclr/new/control/base_reward_plot_{env_name}_{seed}{suffix}.png'
...
if __name__ == "__main__":
    ...
    parser.add_argument('--task', type=str, choices=['cartpole', 'acrobot', 'mountaincar'], 
                       default='cartpole', help='Choose task: cartpole, acrobot, or mountaincar')
    ...
    if args.task == 'cartpole':
        test_cartpole(levels, seed, no_noise)
        print(f"Tested CartPole with {levels} levels and seed {seed}" + (" (no noise)" if no_noise else ""))
    elif args.task == 'acrobot':
        test_acrobot(levels, seed, no_noise)
        print(f"Tested Acrobot with {levels} levels and seed {seed}" + (" (no noise)" if no_noise else ""))
    elif args.task == 'mountaincar':
        test_mountaincar(levels, seed, no_noise)
        print(f"Tested MountainCar with {levels} levels and seed {seed}" + (" (no noise)" if no_noise else ""))
