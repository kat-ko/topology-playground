...
from src.utils.task_normalization import (
    ...
    get_task_thresholds, get_normalization_constants, calculate_reward_completion_percentage
)
...
def double_task_training(policy_class, topology_type, config, num_layers=2, hidden_size=None, train_task_1=None, train_task_2=None):
    ...
    if wandb.run is None:
        ...
        run_name = create_run_name(config, topology_type, 'double_task')
        tags = create_run_tags(config, topology_type, 'double_task')
    ...
    if wandb.run is not None and 'target_capacity' not in config:
        try:
            ...
            updated_run_name = create_run_name(config, topology_type, 'double_task', model)
