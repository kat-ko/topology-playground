...
class DebugTopologyPolicy(ActorCriticPolicy):
    ...
    def __init__(self, observation_space, action_space, lr_schedule, topology_type='fully_connected', hidden_size=384, num_layers=3, config=None, *args, **kwargs):
    ...
    def _create_topology_network(self, network_type):
        ...
        if self.topology_type == 'fully_connected':
            ...
            num_modules = getattr(wandb.config, 'modular_num_modules', 8) if wandb.run else 8
            ...
            num_modules = getattr(wandb.config, 'hybrid_num_modules', 8) if wandb.run else 8
        ...
        try:
            ...
            if hasattr(topology_network, 'node_states'):
                ...
                if hasattr(topology_network, 'generate'):
                    ...
                    hidden_size = getattr(topology_network, 'size', 384)
...
def create_continual_learning_run_name(config, topology_type, task_name, seed, model=None, shift_range=None, no_noise=False):
    ...
    Example: SW_L3_S384_P12345_CP_seed42_L15_I3000_LS200_N02
    """
    # Topology abbreviation
    topology_abbrev = {
        'small_world': 'SW',
    ...
    if model is not None and hasattr(model, 'policy'):
        try:
            ...
            if topology_type == 'small_world':
                ...
                num_modules = config.get('modular_num_modules', 8)
                ...
                num_modules = config.get('hybrid_num_modules', 8)
