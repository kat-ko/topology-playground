...
class LocalDataCollector:
    ...
    def __init__(self, base_path="test_experiments/MODULARITY", run_id=None):
...
class Figure6Plotter:
    ...
    def __init__(self, data_path="test_experiments/MODULARITY"):
...
class ContinualLearningWrapper(gym.Wrapper):
    ...
    def __init__(self, env, task_name, max_iterations=3000, level_switch=200, shift_range=[0, 2], seed=None, reward_scale=20.0, episode_cap=400, logging_callback=None, num_levels=3, no_noise=False):
        super().__init__(env)
...
def make_env(env_name, seed=None, continual_learning=False, max_iterations=3000, level_switch=200, shift_range=[0, 1], reward_scale=20.0, episode_cap=400, logging_callback=None, num_levels=3, no_noise=False):
...
def create_debug_config(num_levels=3, num_layers=1):
...
def continual_learning_training(config, task_name, topology_type, seed, use_wandb=True, enable_phase3=False, device=None, no_noise=False, save_final_model=False, checkpoint_dir=None):
    """
    ...
    if use_wandb and wandb.run is not None:
        ...
        local_data_collector = LocalDataCollector(
            base_path="test_experiments/MODULARITY",
    ...
    if save_final_model:
        if checkpoint_dir is None:
            # Save in the same directory as training data
            if hasattr(local_data_collector, 'run_dir'):
                checkpoint_dir = local_data_collector.run_dir
            else:
                # Fallback to old location if no data collector
        ...
        metadata = {
            ...
            "level_switch": config.get('level_switch', 200),
            "total_iterations": config.get('max_iterations', 3000)
        }
        ...
        print(f"📋 Metadata saved: {checkpoint_dir}/training_metadata.json")
...
def analyze_functional_modularity(task_name, topology_type, seed, num_episodes=100, num_levels=15, evaluation_seeds=None, checkpoint_dir=None):
    """
    Complete Experimental Protocol: Functional vs Structural Modularity Analysis
    
    Following the experimental protocol:
    Step 1: Collect Hidden Activations (fixed evaluation set)
    Step 2: Build Functional Connectivity (FC) matrix
    Step 3: Detect Functional Modules 
    Step 4: Structural Modularity Analysis
    Step 5: Compare FC vs Structural modularity
    """
    print(f"\n🧠 COMPLETE MODULARITY ANALYSIS PROTOCOL")
    print(f"   Task: {task_name}, Topology: {topology_type}, Seed: {seed}")
    print(f"   Episodes per level: {num_episodes}")
    
    # Load model and metadata - use provided checkpoint_dir if available
    if checkpoint_dir is not None:
        model_path = f"{checkpoint_dir}/final_model.zip"
        metadata_path = f"{checkpoint_dir}/training_metadata.json"
        print(f"✅ Using provided model path: {model_path}")
    else:
        # Fallback to search logic if no checkpoint_dir provided
        import glob
        training_data_pattern = f"test_experiments/MODULARITY/*{topology_type}*{task_name}*seed{seed}*"
        matching_dirs = glob.glob(training_data_pattern)
        
        model_path = None
        metadata_path = None
    
        # Try new location first (in training data directory)
        if matching_dirs:
            for dir_path in matching_dirs:
                potential_model = f"{dir_path}/final_model.zip"
                potential_metadata = f"{dir_path}/training_metadata.json"
                if os.path.exists(potential_model):
                    model_path = potential_model
                    metadata_path = potential_metadata
                    print(f"✅ Found model in training data directory: {dir_path}")
                    break
        
        # Fallback to old location
        if model_path is None:
            old_checkpoint_dir = f"modularity_checkpoints/{topology_type}_{task_name}_seed{seed}"
            model_path = f"{old_checkpoint_dir}/final_model.zip"
            metadata_path = f"{old_checkpoint_dir}/training_metadata.json"
            if os.path.exists(model_path):
                print(f"✅ Found model in legacy checkpoint directory: {old_checkpoint_dir}")
            else:
                print(f"❌ No model found in either location")
    ...
    print(f"   Training: {metadata['num_levels']} levels, {metadata['total_iterations']} iterations")
    ...
    # STEP 4: STRUCTURAL MODULARITY ANALYSIS (do this once per model)
    print(f"\n🏗️  STEP 4: STRUCTURAL MODULARITY ANALYSIS")
    structural_results = analyze_structural_modularity(model)
    print(f"   ✅ Structural Q: {structural_results['modularity_score']:.4f}")
    print(f"   ✅ Structural Communities: {structural_results['num_communities']}")
    
    # Generate exact perturbations as used during training
    level_switch = metadata['level_switch']
    shift_range = metadata.get('shift_range', [0, 1])
    perturbation_rng = np.random.RandomState(seed)
    obs_dim = 4 if task_name == "CartPole-v1" else (6 if task_name == "Acrobot-v1" else 8)
    perturbations = []
    
    effective_levels = max(1, metadata['num_levels'])
    for level in range(effective_levels):
        if level == 0:
            perturbation = np.zeros(obs_dim)
        else:
            perturbation = perturbation_rng.normal(shift_range[0], shift_range[1], obs_dim)
        perturbations.append(perturbation)
    
    print(f"✅ Generated {len(perturbations)} perturbation levels using seed {seed}")
    
    # FUNCTIONAL MODULARITY ANALYSIS PER LEVEL
    functional_results = []
    
    for level in range(min(num_levels, len(perturbations))):
        print(f"\n📊 LEVEL {level} ANALYSIS")
        print("=" * 40)
        
        noise_vector = perturbations[level]
        if level == 0:
            noise_description = "Clean Baseline"
            print(f"🧹 {noise_description} (NO NOISE)")
        else:
            noise_description = f"Level {level} Perturbation"
            print(f"🎯 {noise_description}")
            print(f"   Noise vector: {noise_vector}")
        
        # STEP 1: COLLECT HIDDEN ACTIVATIONS (Level-specific with fixed seeds)
        print(f"\n🔬 STEP 1: Collecting Hidden Activations")
        activations = collect_level_activations(
            model, task_name, level, noise_vector, num_episodes
        )
        
        if activations is None or activations.shape[0] == 0:
            print(f"   ❌ No activations collected for level {level}")
            continue
        
        print(f"   ✅ Collected: {activations.shape} (T={activations.shape[0]} timesteps, N={activations.shape[1]} neurons)")
        
        # STEP 2: BUILD FUNCTIONAL CONNECTIVITY (FC)
        print(f"\n🔗 STEP 2: Building Functional Connectivity Matrix")
        fc_matrix = build_functional_connectivity_matrix(activations, correlation_threshold=0.05)
        
        # STEP 3: DETECT FUNCTIONAL MODULES
        print(f"\n🔍 STEP 3: Detecting Functional Modules")
        func_communities, func_modularity, num_func_communities, func_sizes = detect_functional_communities(fc_matrix, graph_threshold=0.1)
        
        # STEP 5: COMPARE FUNCTIONAL vs STRUCTURAL
        print(f"\n⚖️  STEP 5: Comparing Functional vs Structural")
        comparison = compare_functional_vs_structural(
            func_communities, structural_results['communities'],
            func_modularity, structural_results['modularity_score']
        )
        
        # Store comprehensive results
        level_result = {
            'level': level,
            'noise_vector': noise_vector.tolist() if noise_vector is not None else None,
            'noise_description': noise_description,
            'functional_modularity': func_modularity,
            'functional_communities': num_func_communities,
            'functional_community_sizes': func_sizes,
            'structural_modularity': structural_results['modularity_score'],
            'structural_communities': structural_results['num_communities'],
            'structural_community_sizes': structural_results['community_sizes'],
            'community_alignment': comparison,
            'num_neurons': activations.shape[1],
            'num_timesteps': activations.shape[0],
            'fc_sparsity': float(np.mean(fc_matrix == 0)),
            'fc_matrix': fc_matrix.tolist(),  # Save for visualization
            'functional_community_assignments': func_communities,
            'structural_community_assignments': structural_results['communities']
        }
        functional_results.append(level_result)
    ...
    analysis_results = {
        'metadata': {
            ...
            'evaluation_seeds': evaluation_seeds[:num_episodes] if evaluation_seeds else None,
            'analysis_timestamp': datetime.now().isoformat(),
            'protocol_version': 'complete_fc_vs_structural'
        },
        'structural_analysis': structural_results,
        'functional_analysis_per_level': functional_results
    }
    
    # Save results
    results_path = f"{checkpoint_dir}/complete_modularity_analysis.json"
    with open(results_path, 'w') as f:
        json.dump(analysis_results, f, indent=2)
    
    # Generate visualizations
    print(f"\n🎨 Generating Visualizations...")
    create_modularity_visualizations(analysis_results, checkpoint_dir)
    
    print(f"\n💾 Complete analysis saved: {results_path}")
...
class ExactNoiseWrapper(gym.Wrapper):
    """Simple wrapper that applies exact noise vector to observations."""
    
    def __init__(self, env, noise_vector):
        super().__init__(env)
        self.noise_vector = np.array(noise_vector) if noise_vector is not None else np.zeros(env.observation_space.shape[0])
    
    def step(self, action):
        obs, reward, terminated, truncated, info = self.env.step(action)
        return obs + self.noise_vector, reward, terminated, truncated, info
    
    def reset(self, **kwargs):
        obs, info = self.env.reset(**kwargs)
        return obs + self.noise_vector, info


def create_perturbed_env_exact(task_name, noise_vector):
    """Create environment with exact noise vector used during training."""
    ...
    # Wrap with exact noise vector
    env = ExactNoiseWrapper(env, noise_vector)
    
    return env


def create_perturbed_env(task_name, noise_magnitude):
    """Create environment with specified noise level (legacy function)."""
    if task_name == "CartPole-v1":
        env = gym.make("CartPole-v1")
    elif task_name == "Acrobot-v1":
        env = gym.make("Acrobot-v1")
    ...
    # Wrap with noise if needed
    if noise_magnitude > 0:
        from src.utils.observation_perturbation import ObservationPerturbationWrapper
        env = ObservationPerturbationWrapper(env, noise_magnitude)
...
def collect_level_activations(model, task_name, level, noise_vector, num_episodes=100):
    ...
    Returns matrix X_k ∈ R^(T×N) where T=timesteps, N=neurons
    """
...
def collect_hidden_activations_fixed_seeds(model, env, evaluation_seeds):
    ...
    for episode_idx, seed in enumerate(evaluation_seeds):
        ...
        while not done and step < max_steps:
            with torch.no_grad():
                obs_tensor = torch.FloatTensor(obs).unsqueeze(0)
                features = model.policy.features_extractor(obs_tensor)
                episode_activations.append(features.cpu().numpy().flatten())
...
def collect_hidden_activations(model, env, num_episodes):
    """Legacy function for backward compatibility."""
    evaluation_seeds = list(range(1000, 1000 + num_episodes))
    return collect_hidden_activations_fixed_seeds(model, env, evaluation_seeds)


def build_functional_connectivity_matrix(activations, correlation_threshold=0.05):
    """
    STEP 2: Build FC matrix from activation time series using Pearson correlations.
...
def compute_functional_connectivity_advanced(activations, threshold=0.05):
    """Legacy function for backward compatibility."""
    return build_functional_connectivity_matrix(activations, threshold)


def compute_functional_connectivity(activations):
    """Legacy function for backward compatibility."""
    return compute_functional_connectivity_advanced(activations, threshold=0.0)


def detect_functional_communities(fc_matrix, graph_threshold=0.1):
    """
    STEP 3: Detect functional modules using Louvain algorithm.
...
def detect_communities_louvain(fc_matrix, threshold=0.1):
    """Legacy function for backward compatibility."""
    communities, modularity_score, num_communities, community_sizes = detect_functional_communities(fc_matrix, threshold)
    return communities, modularity_score


def analyze_structural_modularity(model):
    """
    STEP 4: Extract structural modularity from network weights.
...
def compare_functional_vs_structural(func_communities, struct_communities, func_q, struct_q):
    """
    STEP 5: Compare functional and structural community structures.
...
def compare_functional_structural_modularity(func_communities, struct_communities, func_q, struct_q):
    """Legacy function for backward compatibility."""
    return compare_functional_vs_structural(func_communities, struct_communities, func_q, struct_q)


def create_modularity_visualizations(analysis_results, checkpoint_dir):
    """
    Generate visualizations as per protocol:
    - FC matrix heatmaps sorted by communities
    - Q_struct vs Q_FC scatterplot
    """
    import matplotlib.pyplot as plt
    import seaborn as sns
    
    # Create plots directory
    plots_dir = f"{checkpoint_dir}/modularity_plots"
    os.makedirs(plots_dir, exist_ok=True)
    
    functional_results = analysis_results['functional_analysis_per_level']
    
    # 1. FC Matrix Heatmaps for each level
    for level_result in functional_results:
        level = level_result['level']
        fc_matrix = np.array(level_result['fc_matrix'])
        
        plt.figure(figsize=(8, 6))
        sns.heatmap(fc_matrix, annot=True, cmap='coolwarm', center=0, 
                   square=True, linewidths=0.5, cbar_kws={"shrink": .8})
        plt.title(f'Functional Connectivity Matrix - Level {level}\n'
                 f'Q_FC = {level_result["functional_modularity"]:.3f}, '
                 f'Communities = {level_result["functional_communities"]}')
        plt.xlabel('Neuron Index')
        plt.ylabel('Neuron Index')
        
        plot_path = f"{plots_dir}/fc_matrix_level_{level}.png"
        plt.savefig(plot_path, dpi=150, bbox_inches='tight')
        plt.close()
        
        print(f"   📊 Saved FC heatmap: {plot_path}")
    
    # 2. Q_struct vs Q_FC comparison plot
    if len(functional_results) > 1:
        q_functional = [r['functional_modularity'] for r in functional_results]
        q_structural = [r['structural_modularity'] for r in functional_results]
        levels = [r['level'] for r in functional_results]
        
        plt.figure(figsize=(10, 6))
        
        # Scatterplot
        plt.subplot(1, 2, 1)
        plt.scatter(q_structural, q_functional, s=100, alpha=0.7, c=levels, cmap='viridis')
        plt.plot([0, max(max(q_structural), max(q_functional))], 
                [0, max(max(q_structural), max(q_functional))], 
                'r--', alpha=0.5, label='Q_FC = Q_struct')
        plt.xlabel('Structural Modularity (Q_struct)')
        plt.ylabel('Functional Modularity (Q_FC)')
        plt.title('Structural vs Functional Modularity')
        plt.legend()
        plt.grid(True, alpha=0.3)
        
        # Level progression
        plt.subplot(1, 2, 2)
        plt.plot(levels, q_functional, 'o-', label='Q_FC', linewidth=2, markersize=8)
        plt.plot(levels, q_structural, 's-', label='Q_struct', linewidth=2, markersize=8)
        plt.xlabel('Level')
        plt.ylabel('Modularity Score (Q)')
        plt.title('Modularity Evolution Across Levels')
        plt.legend()
        plt.grid(True, alpha=0.3)
        
        plt.tight_layout()
        plot_path = f"{plots_dir}/modularity_comparison.png"
        plt.savefig(plot_path, dpi=150, bbox_inches='tight')
        plt.close()
...
def print_comprehensive_summary(analysis_results):
    """
    Print comprehensive summary of the complete analysis.
    """
    print(f"\n" + "="*70)
    print(f"📊 COMPREHENSIVE MODULARITY ANALYSIS SUMMARY")
    print(f"="*70)
    
    metadata = analysis_results['metadata']
    structural = analysis_results['structural_analysis']
    functional = analysis_results['functional_analysis_per_level']
    
    print(f"🎯 Analysis Details:")
    print(f"   Task: {metadata['task_name']}")
    print(f"   Topology: {metadata['topology_type']}")
    print(f"   Seed: {metadata['seed']}")
    print(f"   Episodes per level: {metadata['num_episodes']}")
    print(f"   Protocol: {metadata['protocol_version']}")
    
    print(f"\n🏗️  STRUCTURAL MODULARITY:")
    print(f"   Q_struct: {structural['modularity_score']:.4f}")
    print(f"   Structural Communities: {structural['num_communities']}")
    print(f"   Matrix Shape: {structural['matrix_shape']}")
    
    print(f"\n🧠 FUNCTIONAL MODULARITY PER LEVEL:")
    print(f"{'Level':<6} {'Noise':<12} {'Q_FC':<8} {'Q_struct':<8} {'ΔQ':<8} {'NMI':<6} {'ARI':<6} {'F_Comm':<6} {'S_Comm':<6}")
    print("-" * 75)
    
    for result in functional:
        level = result['level']
        noise = "Clean" if result['level'] == 0 else f"L{result['level']}"
        q_fc = result['functional_modularity']
        q_struct = result['structural_modularity']
        delta_q = result['community_alignment']['q_difference']
        nmi = result['community_alignment']['nmi']
        ari = result['community_alignment']['ari']
        f_comm = result['functional_communities']
        s_comm = result['structural_communities']
        
        print(f"{level:<6} {noise:<12} {q_fc:<8.4f} {q_struct:<8.4f} {delta_q:<8.4f} {nmi:<6.3f} {ari:<6.3f} {f_comm:<6} {s_comm:<6}")
    
    # Key insights
    print(f"\n🔍 KEY INSIGHTS:")
    
    avg_q_fc = np.mean([r['functional_modularity'] for r in functional])
    avg_q_struct = structural['modularity_score']
    
    print(f"   Average Q_FC: {avg_q_fc:.4f}")
    print(f"   Structural Q: {avg_q_struct:.4f}")
    print(f"   FC > Structural: {'✅' if avg_q_fc > avg_q_struct else '❌'}")
    
    avg_nmi = np.mean([r['community_alignment']['nmi'] for r in functional])
    avg_ari = np.mean([r['community_alignment']['ari'] for r in functional])
    
    print(f"   Average Community Alignment (NMI): {avg_nmi:.3f}")
    print(f"   Average Community Alignment (ARI): {avg_ari:.3f}")
    print(f"   Strong Alignment (NMI > 0.5): {'✅' if avg_nmi > 0.5 else '❌'}")
    
    print(f"\n💡 INTERPRETATION:")
    if avg_q_fc > 0.3:
        print(f"   🟢 STRONG functional modularity detected")
    elif avg_q_fc > 0.1:
        print(f"   🟡 MODERATE functional modularity detected") 
    else:
        print(f"   🔴 WEAK functional modularity detected")
        
    if avg_nmi > 0.7:
        print(f"   🟢 HIGH alignment between functional and structural communities")
    elif avg_nmi > 0.3:
        print(f"   🟡 MODERATE alignment between functional and structural communities")
    else:
        print(f"   🔴 LOW alignment between functional and structural communities")
...
def run_test_experiment(task_name="CartPole-v1", seeds=[42, 123, 456, 789, 999], use_wandb=False, num_levels=3, device=None, no_noise=False):
    """
    ...
    plotter = Figure6Plotter(data_path="test_experiments/MODULARITY")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_model", action="store_true", help="Save trained model for later analysis")
    parser.add_argument("--analysis", action="store_true", help="Perform functional modularity analysis on saved models")
    ...
    if args.test:
        ...
        # Check if we need to train or just analyze
        # Check both new location (training data) and old location (modularity_checkpoints)
        import glob
        
        # Try multiple patterns to match different naming conventions
        patterns = [
            f"test_experiments/MODULARITY/*{args.topology}*{args.task}*seed{args.seed}*",
            f"test_experiments/MODULARITY/*{args.topology.upper()}*{args.task.replace('-', '')}*seed{args.seed}*",
            f"test_experiments/MODULARITY/*{args.topology.upper()}*CP*seed{args.seed}*",  # CartPole abbreviated as CP
            f"test_experiments/MODULARITY/*STANDARD_MLP*seed{args.seed}*" if args.topology == "standard_mlp" else None
        ]
        ...
        for dir_path in matching_dirs:
            if os.path.exists(f"{dir_path}/final_model.zip"):
                model_exists = True
                checkpoint_dir = dir_path
                break
        
        # Check old location if not found
        if not model_exists:
            old_checkpoint_dir = f"modularity_checkpoints/{args.topology}_{args.task}_seed{args.seed}"
            if os.path.exists(f"{old_checkpoint_dir}/final_model.zip"):
                model_exists = True
                checkpoint_dir = old_checkpoint_dir
        
        # If only analysis is requested and model exists, skip training
        if args.analysis and not args.save_model and model_exists:
            print("🧠 Analysis-only mode: Skipping training (model already exists)")
            print(f"   Using existing model: {checkpoint_dir}/final_model.zip")
            
            # Run functional modularity analysis
            print("\n" + "="*60)
            print("🧠 FUNCTIONAL MODULARITY ANALYSIS")
            print("="*60)
            
            try:
                analysis_results = analyze_functional_modularity(
                    task_name=args.task,
                ...
                if analysis_results:
                    print("✅ Functional modularity analysis completed successfully!")
                else:
                    print("❌ Analysis failed - check if model was saved correctly")
            ...
            # Run training (and optionally analysis)
            ...
            try:
                ...
                # Run functional modularity analysis if requested
                if args.analysis:
                    print("\n" + "="*60)
                    print("🧠 FUNCTIONAL MODULARITY ANALYSIS")
                    print("="*60)
                    ...
                    try:
                        ...
                        if analysis_results:
                            ...
                            print("❌ Analysis failed - check if model was saved with --save_model")
